...
function extractMultipleClientsFromSingleLine(line: string): ExtractedClient[] {
  ...
  phoneMatches.forEach((phoneMatch, index) => {
    ...
    if (index > 0) {
      ...
      contextStart =
        phoneMatches[index - 1].index! + phoneMatches[index - 1][0].length;
    }
    ...
    const clientContext = line.substring(
      contextStart,
      phoneIndex + phone.length,
    );
    ...
    const beforeEmail = clientContext
      .substring(0, clientContext.indexOf(email))
      .trim();
...
function detectGenderIntelligently(
  ...
  if (
    ...
    !["garcia", "costa", "silva"].includes(firstName)
  ) {
